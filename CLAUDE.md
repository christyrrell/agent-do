...
| `agent-media/` | Bash + Python | Unified media family surface: rip optical discs to MKV and convert video to Plex-ready MP4 through neutral verbs (`drives`, `scan`, `rip`, `convert`, `batch`, `presets`, `verify`, `snapshot`). Providers live under `tools/agent-media/providers/` (makemkv for discs, handbrake for transcodes), never as top-level tools; each engine's dependency, readiness, and disc-specific language stay at the provider layer, and `media makemkv <cmd>` / `media handbrake <cmd>` passthroughs keep the full provider surface reachable. `snapshot` and `providers` degrade per provider with the reason. |
